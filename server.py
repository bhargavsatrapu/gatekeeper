...
def get_database_tables():
    ...
    try:
        ...
        result = testcase_repo.get_all_tables_info()
        return result
...
def clear_database():
    ...
    try:
        # Get test case repository (which has the clear_all_tables method)
        testcase_repo = get_test_case_repository()
        
        # Clear ALL tables in the database
        result = testcase_repo.clear_all_tables()
        
        logger.info(f"Database cleared: {result}")
        
        # Verify that tables are actually cleared
        if result.get("status") == "success":
            verification = testcase_repo.verify_tables_cleared()
            result["verification"] = verification
            
            if not verification.get("all_tables_empty", False):
                logger.warning(f"Tables not fully cleared: {verification}")
                result["warning"] = "Some tables may not have been fully cleared"
...
def verify_database_cleared():
    """Verify that all tables are empty."""
    try:
        testcase_repo = get_test_case_repository()
        result = testcase_repo.verify_tables_cleared()
        return result
...
def generate_testcases():
    ...
    if not mapping or len(mapping) == 0:
        ...
        return {
            "status": "warning",
            "message": "No test cases were generated. The LLM may have returned invalid JSON or no response.",
            "generated_total": 0,
            "endpoints_processed": len(endpoints),
            "suggestion": "Try again or check the LLM configuration"
        }
